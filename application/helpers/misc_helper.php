...
if (!function_exists('Error_404')) {
    ...
    function Error_404($data = null)
    {
        $CI = get_instance();
        $data['page_title'] = trans("0268");
        $CI->theme->view('404', $data);
...
if (!function_exists('pt_main_module_available')) {
    ...
    function pt_main_module_available($module)
    {
        ...
        if (in_array($module, $allmods) && $enabled) {
            return true;
        } else {
            return false;
        }
...
if (!function_exists('languageName')) {
    ...
    function languageName($language = "vi")
    {
        $CI = get_instance();
        $file = file_get_contents("application/language/$language/name.txt", true);
        $name = explode(",", $file);
        ...
        if (empty($couponcode) || empty($module) || empty($itemid)) {
            ...
            if ($foreverCheck->num_rows() > 0 || $expiryCheck->num_rows() > 0) {
                ...
                if ($totalRemain > 0) {
                    ...
                    $where = "(item = 'all' OR item = $itemid)";
                    $CI->db->where($where);
                    $CI->db->where('couponid', $couponid);
                    $nums = $CI->db->get('pt_coupons_assign')->num_rows();
                    if ($nums > 0) {

                        $response = array('status' => 'success', 'couponid' => $couponid, 'value' => round($value, 1), 'msg' => "", 'type' => $type);

                    } else {

                        $response = array('status' => 'irrelevant', 'couponid' => "", 'value' => "", 'msg' => "Không thể áp dụng mã giảm giá này.");

                    }

                } else {

                    $response = array('status' => 'fail', 'couponid' => "", 'value' => "", 'msg' => "Mã giảm giá không hợp lệ");
...
if (!function_exists('pt_applyCouponDiscount')) {
    ...
    function pt_applyCouponDiscount($couponid, $amount)
    {
        ...
        if (!empty($res)) {
            ...
            $value = $res[0]->value;
            $type = $res[0]->type;
            if ($type == "VND") $amt = $value;
            else $amt = $amount * ($value / 100);
            $newAmt = $amount - $amt;
            $code = $couponid;
...
if (!function_exists('phpTravelsObj')) {

    function phpTravelsObj()
    {
...
if (!function_exists('get_widget_content')) {

    function get_widget_content($id)
    {
        $CI = get_instance();
        $CI->load->model('admin/widgets_model');
        return $CI->widgets_model->get_widget_content($id);
    }

}
if (!function_exists('get_widget_freshcontent')) {

    function get_widget_freshcontent($id)
    {
        $CI = get_instance();
        $CI->load->model('admin/widgets_model');
        return $CI->widgets_model->get_widget_freshcontent($id);
    }
...
if (!function_exists('get_widget_title')) {
    ...
    function get_widget_title($id)
    {
        $CI = get_instance();
        $CI->load->model('admin/widgets_model');
        return $CI->widgets_model->get_widget_title($id);
    }
...
if (!function_exists('get_widget_name')) {
    ...
    function get_widget_name($id)
    {
        $CI = get_instance();
        $CI->load->model('admin/widgets_model');
        return $CI->widgets_model->get_widget_name($id);
    }

}
if (!function_exists('get_widget_id')) {

    function get_widget_id($location_id, $locate = null)
    {
        $CI = get_instance();
        $CI->load->model('admin/widgets_model');
        return $CI->widgets_model->get_widget_id($location_id, $locate);
    }

}
if (!function_exists('get_widget_ids')) {

    function get_widget_ids($location_id, $locate = null)
    {
        $CI = get_instance();
        $CI->load->model('admin/widgets_model');
        return $CI->widgets_model->get_widget_ids($location_id, $locate);
    }

}
if (!function_exists('get_location_image')) {
    ...
    function get_location_image($location_id)
    {
        ...
        $content = $CI->db->get('pt_locations')->result();
        if (!empty($content)) {
...
if (!function_exists('get_location_hotel_image')) {

    function get_location_hotel_image($location_id)
    {
        $CI = get_instance();
        $CI->db->where('id', $location_id);
...
//format a date from database which is YYYY-MM-DD format to application settings format
    function getPayment($gateway)
    {
        $CI = get_instance();
        $CI->db->where('gateway', $gateway);
        $CI->db->where('setting', 'name');
        $paymentname = $CI->db->get('pt_paymentgateways')->result();

        return $paymentname[0]->value;

    }

}
if (!function_exists('getBookingPaymentinfo')) {
//format a date from database which is YYYY-MM-DD format to application settings format
    function getBookingPaymentinfo($bookingID)
    {
        $CI = get_instance();
        $CI->db->where('booking_id', $bookingID);
        $bookingpaymentinfo = $CI->db->get('pt_bookings')->result();
        $booking_payment_info = $bookingpaymentinfo[0]->booking_payment_info;
          $CI = get_instance();
        $CI->db->where('gateway', 'banktransfer');
        $CI->db->where('setting',$booking_payment_info);
        $paymentname = $CI->db->get('pt_paymentgateways')->result();
        return $paymentname[0]->value;
...
//get locations list admin panel
    ...
    function getLocations($national = null)
    {
        $CI = get_instance();
        $CI->db->where('status', 'yes');
        if (!empty($national)) {
            $CI->db->where('country', $national);
        }
        $CI->db->order_by('location', 'asc');
        return $CI->db->get('pt_locations')->result();
...
//get locations list admin panel
    ...
    function getLocationsdesc($id)
    {
        $CI = get_instance();
        $CI->db->where('id', $id);
        $query = $CI->db->get('pt_locations');
        $row = $query->row();
        return $row->description;

    }
}
if (!function_exists('getCamnang')) {
//get locations list admin panel

    function getCamnang($id)
    {
        $CI = get_instance();
        $CI->db->where('location', $id);
        $query = $CI->db->get('pt_blog');
        $row = $query->row();
