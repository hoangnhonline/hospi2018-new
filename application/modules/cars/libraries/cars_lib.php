...
//set car id by id
		...
		function wishListInfo($id) {
			   $this->db->select('car_title,car_slug,thumbnail_image,car_city,car_stars');
			   $this->db->where('car_id',$id);
			   $result = $this->db->get('pt_cars')->result();
			   $title = $this->get_title($result[0]->car_title);
			   $slug = "cars/".$result[0]->car_slug;
			   $thumbnail = PT_CARS_SLIDER_THUMB.$result[0]->thumbnail_image;
			   
			   $location = pt_LocationsInfo($result[0]->car_city, $this->lang);
			   
			   $stars = pt_create_stars($result[0]->car_stars);
			   $res = array(
			   	"title" => $title, 
			   	"slug" => $slug,
			   	"thumbnail" => $thumbnail,
		...
		function show_cars($offset = null) {
			$this->ci->load->library('bootpagination');
				...
				$data['paginationinfo'] = array('base' => 'cars/listing', 'totalrows' => $rh['rows'], 'perpage' => $perpage);
				$data['plinks2'] = $this->ci->bootpagination->dopagination2('cars/listing', $rh['rows'], $perpage);
				return $data;
		}
...
// show cars listings on front page based on locations
		...
		function search_cars_by_text($offset = null, $cities = null) {
				...
				$data['plinks'] = $this->ci->bootpagination->dopagination('cars/search', $rh['rows'], $perpage);
				return $data;
		}
...
// get car images
        ...
        $curr = $this->ci->currconverter;
				...
				$details = $this->db->get('pt_cars')->result();
     			$title = $this->get_title($details[0]->car_title,$details[0]->car_id);
                ...
                $detailResults = (object)array('id' => $details[0]->car_id, 'title' => $title, 'slug' => $slug, 'bookingSlug' => $bookingSlug,'thumbnail' => $thumbnail,
                 ...
                 'carPrice' => $carPrice,'totalDeposit' => $depositAmount, 'taxAmount' => $this->taxamount, 'showTotal' => $showTotal,'pickupLocation' => $this->pickupLocation,'dropoffLocation' => $this->dropoffLocation, 'pickupTime' => $this->pickupTime, 'pickupDate' => $this->pickupDate,'dropoffTime' => $this->dropoffTime, 'dropoffDate' => $this->dropoffDate, 'totalDays' => $this->totalDays);
		...
		function car_short_details($carid = null) {
			...
			if(empty($carid)){
				...
				$this->db->select('car_title,car_slug,car_location,car_type,car_stars,car_desc,car_policy,car_is_featured,car_basic_price,car_basic_discount,car_city,car_latitude,car_longitude,thumbnail_image,car_passengers,car_doors,car_baggage,car_transmission,car_airport_pickup');
				$this->db->where('car_id', $carid);
				$details = $this->db->get('pt_cars')->result();
		...
		function getCarPriceOnChangeLocation($carid, $pickup,$dropoff,$pickupDate, $dropoffDate){
         ...
         function getLimitedResultObject($cars){
          ...
          if(!empty($cars)){
            ...
            'stars' => pt_create_stars($this->stars), 'location' => $this->location,'price' => $price,'currCode' => $curr->code,'currSymbol' => $curr->symbol,'latitude' => $this->latitude, 'longitude' => $this->longitude,'avgReviews' => $avgReviews,'totalDays' => $this->totalDays);
            }
		...
		function carReviewsAvg($carid = null) {
         ...
         function getResultObject($cars){
          ...
          foreach($cars as $c){
            ...
            'transmission' => $this->transmission,'airportPickup' => $this->aiportPickup,'baggage' => $this->baggage,'price' => $price,'currCode' => $curr->code,'currSymbol' => $curr->symbol,'avgReviews' => $avgReviews, 'latitude' => $this->latitude,'longitude' => $this->longitude,'carType' => $this->carType);
            }
