...
redirect('admin');
		...
		function index() {
                ...
                $xcrud->multiDelUrl = base_url().'tours/tourajaxcalls/delMultipleTours';
				...
				$xcrud->columns('tour_is_featured,thumbnail_image,tour_title,tour_stars,Owned By,tour_slug,tour_order,tour_status');

				$xcrud->search_columns('tour_title,tour_stars,Owned By,tour_order,tour_status');
		...
		function add() {
			...
			if(!$this->data['addpermission']){
				...
				if (!empty ($addtour)) {
						$this->form_validation->set_rules('tourname', 'Tour Name', 'trim|required|xss_clean');
