...
//set tour id by id
    ...
    function tour_details($tourid = null, $date = null) {
        ...
        $departure = $details[0]->tour_departure;
        $transportation = $details[0]->tour_transportation;
        $include = $details[0]->include;
        $exclude = $details[0]->exclude;
        $tourcode = $details[0]->tour_code;
        ...
        $address = $details[0]->tour_fulladdress;
        $operator = $details[0]->tour_operator;
        ...
        $detailResults = (object) array('id' => $details[0]->tour_id, 'title' => $title, 'slug' => $slug, 'bookingSlug' => $bookingSlug, 'thumbnail' => $thumbnail,'departure' => $departure,'transportation' => $transportation,
                    'stars' => pt_create_stars($stars), 'starsCount' => $stars, 'location' => $location, 'desc' => $desc, 'inclusions' => $inclusions, 'exclusions' => $exclusions,'include' => $include,'exclude' => $exclude,
                    'latitude' => $latitude, 'longitude' => $longitude, 'sliderImages' => $sliderImages, 'relatedItems' => $relatedTours, 'paymentOptions' => $paymentOptions,'tourcode' => $tourcode,'address' => $address,'operator' => $operator,
                    ...
                    'perAdultPrice' => $perAdultPrice, 'perChildPrice' => $perChildPrice, 'perInfantPrice' => $perInfantPrice, 'currCode' => $curr->code, 'currSymbol' => $curr->symbol, 'date' => $this->date, 'totalCost' => $curr->convertPrice($totalCost),
                    'comType' => $comm_type, 'comValue' => $comm_value, 'taxType' => $tax_type, 'taxValue' => $tax_value, 'tourDays' => $tourDays,
                    'tourNights' => $tourNights, 'totalDeposit' => $depositAmount, 'mapAddress' => $details[0]->tour_mapaddress,'countryName' => $countryName);
    ...
    function tour_short_details($tourid) {
        ...
        $this->db->select('tour_title,tour_stars,tour_slug,tour_desc,tour_privacy,tour_max_adults,tour_max_child,tour_departure,tour_transportation,include,exclude,tour_code,
   tour_max_infant,tour_basic_price,tour_basic_discount,tour_adult_price,tour_child_price,tour_infant_price,tour_amenities,tour_exclusions,tour_days,tour_nights,thumbnail_image,tour_location,tour_latitude,tour_longitude,tour_type');
        ...
        $this->departure = $details[0]->tour_departure;
        $this->transportation = $details[0]->tour_transportation;
        $this->include = $details[0]->include;
        $this->exclude = $details[0]->exclude;
        $this->tourcode = $details[0]->tour_code;
    ...
    function updatedPrice($tourid, $adults = 1, $child = 0, $infant = 0) {
        ...
        $detailResults = array('id' => $tourid, 'adultPrice' => $adultPrice, 'childPrice' => $childPrice, 'infantPrice' => $infantPrice,
            'currCode' => $curr->code, 'currSymbol' => $currSymbol, 'totalDeposit' => $curr->convertPrice($depositAmount), 'totalCost' => $curr->convertPrice($totalCost));
...
// Tour visiting Cities
    ...
    function getResultObject($tours) {
        ...
        foreach ($tours as $t) {
            ...
            $result[] = (object) array('id' => $this->tourid, 'title' => $this->title, 'slug' => base_url() . 'tours/' . $this->slug, 'thumbnail' => $this->thumbnail,
                        'stars' => pt_create_stars($this->stars), 'starsCount' => $this->stars, 'location' => $this->location, 'desc' => strip_tags($this->desc), 'transportation' => $this->transportation, 'departure' => $this->departure,
                        'price' => $price, 'currCode' => $curr->code, 'currSymbol' => $curr->symbol, 'inclusions' => $this->inclusions,'include' => $this->include,'exclude' => $this->exclude,'tourcode' => $this->tourcode,
        ...
        if (!empty($tours)) {
            foreach ($tours as $t) {
                ...
                if (!empty($this->title)) {
                    ...
                    $result[] = (object) array('id' => $this->tourid, 'title' => $this->title, 'slug' => base_url() . 'tours/' . $this->slug, 'thumbnail' => $this->thumbnail, 'departure' => $this->departure,
    ...
    function getBookResultObject($tourid, $date = null, $adults = null, $child = null, $infants = null) {
        ...
        $result["tour"] = (object) array('id' => $this->tourid, 'title' => $this->title, 'slug' => base_url() . 'tours/' . $this->slug, 'thumbnail' => $this->thumbnail,
                    'stars' => pt_create_stars($this->stars), 'starsCount' => $this->stars, 'location' => $this->location, 'date' => $date, 'transportation' => $this->transportation, 'departure' => $this->departure,
                    ...
                    'adultprice' => $curr->convertPrice($adultPrice), 'childprice' => $curr->convertPrice($childPrice), 'infantprice' => $curr->convertPrice($infantPrice), 'subTotal' => $subTotal);
    ...
    public function toursByLocations($totalnums = 8) {
        $locData = new stdClass;
        $this->db->select('location_id,tour_id,country');
        $this->db->from('pt_tour_locations');
        $this->db->where('position', '2');
        $this->db->join('pt_locations', 'pt_locations.id = pt_tour_locations.location_id');
        $this->db->group_by('location_id');
        ...
        foreach ($result as $rs) {
            ...
            $locData->locations[] = (object) array('id' => $rs->location_id, 'name' => $locationInfo->city, 'count' => count($tours), 'tours' => $tours, 'country' => $rs->country);
        }
    ...
    public function librandomTours($totalnums = 1) {
        $locData = new stdClass;

        $this->db->select('tour_id,tour_title,thumbnail_image,tour_location,pt_locations.country as tcountry, tour_slug');
        $this->db->from('pt_tours');
        $this->db->join('pt_locations', 'pt_locations.id = pt_tours.tour_location');
        $this->db->where('tour_status', 'Yes');
        $this->db->order_by('pt_tours.tour_id', 'RANDOM');
        $this->db->limit($totalnums);
        $qtours = $this->db->get();
        $tours = $qtours->result();
        $images = array();
        foreach ($tours as $tour) {
            $locationInfo = pt_LocationsInfo($tour->tour_location, $this->lang);
            array_push($images, $tour->thumbnail_image);
            $thumbnail_image = $images[array_rand($images)];
            $locData->locations[] = (object) array('id' => $tour->tour_location, 'title' => $tour->tour_title,'name' => $locationInfo->city, 'country' => $tour->tcountry, 'thumbnail_image' => $thumbnail_image, 'slug' => $tour->tour_slug);
            unset($images);
            $images = array();
        }
    ...
    public function suggestionResults($query) {
        ...
        if (!empty($res)) {
            ...
            foreach ($res as $r) {
                ...
                $tours[] = (object) array('id' => $r->id, 'name' => trim($title), 'module' => 'tour');
            }
        }

        $response = array_merge($tours, $locations);
