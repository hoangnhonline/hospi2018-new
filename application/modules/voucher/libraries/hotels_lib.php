		...
		function show_hotels($offset = null) {
			$this->ci->load->library('bootpagination');
			  ...
			  //	$data['all_hotels'] = $this->ci->hotels_model->list_hotels_front($perpage, $offset, $orderby);
                ...
                $data['paginationinfo'] = array('base' => 'hotels/listing', 'totalrows' => $rh['rows'], 'perpage' => $perpage);
			...
			$this->ci->load->library('bootpagination');
			  ...
			  //	$data['all_hotels'] = $this->ci->hotels_model->list_hotels_front($perpage, $offset, $orderby);
                ...
                $data['paginationinfo'] = array('base' => 'hotels/honeymoonlisting', 'totalrows' => $rh['rows'], 'perpage' => $perpage);
...
// get hotel rooms with limited details
 ...
 // Room Price
		...
		function hotel_details( $hotelid=null ) {
                    ...
                    } else $this->db->where('hotel_id', $this->hotelid);
			   ...
			   //	$this->isfeatured = $this->is_featured();
                ...
                $detailResults = (object)array('id' => $details[0]->hotel_id, 'title' => $title, 'slug' => $slug,'bookingSlug' => $bookingSlug, 'thumbnail' => $thumbnail, 'locationInfoUrl' => $locationInfoUrl, 'cityName' => $cityName,
            'stars' => pt_create_stars($stars), 'starsCount' => $stars, 'location' => $city->city,'desc' => $desc,'amenities' => $amenities,'vatvalue'=>$taxvalue, 'servicevalue'=>$servicevalue,'price_status' =>  $details[0]->price_status,
            'latitude' => $latitude,'longitude' => $longitude,'sliderImages' => $sliderImages,'relatedItems' => $relatedHotels,'paymentOptions' => $paymentOptions,'hotel_city' => $details[0]->hotel_city,'near' => $near,
            'defcheckin' => $defcheckin, 'defcheckout' => $defcheckout,'metadesc' => $metadesc,'keywords' => $keywords,'policy' => $policy,'tripadvisorid' => $tripadvisorid,'mapAddress' => $details[0]->hotel_map_city);
...
// hotel Reviews Averages
		function hotelReviewsAvg($hotelid=null) {
                ...
                $overall = round($res[0]->overall,1);
                 $result = (object)array('clean' => $clean,'comfort' => $comfort,'location' => $location,'facilities' => $facilities,'staff' => $staff,'anuong' => $an_uong,'totalReviews' => $totalreviews,'overall' => $overall );
                 return $result;
		}
		...
		function featured_hotels_list($location=null,$lim=null) {
				...
				else $limit = $settings[0]->front_homepage;
				$orderby = $settings[0]->front_homepage_order;
...
// List 2 top rated hotels from each city passed to to this function
		...
		$curr = $this->ci->currconverter;
          ...
          $this->ci->load->model('hotels/rooms_model');
           ...
           //  $result[] = $p;
            ...
            //if(!empty($result)) {
            //print_r(array_filter($result, function($value) { return $value !== ''; })) ;
						if($list=='yes'){
	            return min($result);
	          } else {
        ...
        function honeymoon_room_price($hotelid) {
         ...
         function getResultObject($hotels,$honeymoon = null){
          $this->ci->load->library('currconverter');
          $result = array();
          $curr = $this->ci->currconverter;
         ...
         if(!empty($_GET['price'])){
			...
			$result[] = (object)array('id' => $this->hotelid, 'title' => $this->title, 'slug' => base_url().'hotels/'.$this->slug,'thumbnail' => $this->thumbnail,'roomid' => $h->room_id,
			'stars' => pt_create_stars($this->stars),'starsCount' => $this->stars, 'location' => $this->location,'mapAddress' => $this->mapAddress,'desc' => strip_tags($this->desc),
			'price' => $price,'currCode' => $curr->code,'currSymbol' => $curr->symbol,'price_status' => $h->price_status,'amenities' => $this->amenities,'vatvalue' => $this->taxvalue, 'servicevalue' => $this->servicevalue,
			'avgReviews' => $avgReviews, 'latitude' => $this->latitude,'longitude' => $this->longitude, 'featuredfrom' => $h->hotel_featured_from, 'featuredto' => $h->hotel_featured_to,
                            'salefrom' => $h->hotel_sale_from, 'saleto' => $h->hotel_sale_to,'salepercent' => $h->hotel_is_sale_percent );
         ...
         function getLimitedResultObject($hotels){
          ...
          if(!empty($hotels)){
            ...
            if(!empty($this->title)){
            	...
            	$result[] = (object)array('id' => $this->hotelid, 'title' => $this->title, 'desc' => $this->desc, 'slug' => base_url().'hotels/'.$this->slug,'thumbnail' => $this->thumbnail,
            'stars' => pt_create_stars($this->stars), 'location' => $this->location,'price' => $price,'currCode' => $curr->code,'currSymbol' => $curr->symbol,'avgReviews' => $avgReviews,'latitude' => $this->latitude,'longitude' => $this->longitude);
        ...
        //make a result object of Rooms Array
         function getRoomsResultObject($rooms,$checkin = null, $checkout = null){
          ...
          foreach($rooms as $room){
            ...
            $Roomresult[] = (object)array('id' => $room->id, 'title' => $this->roomtitle, 'desc' => $this->roomdesc,
            'maxAdults' => $details[0]->room_adults,'maxChild' => $details[0]->room_children,'breakfast' => $breakfast,'onsale' => $onsale,
            'maxQuantity' => $availQuantity, 'thumbnail' => PT_ROOMS_THUMBS.$details[0]->thumbnail_image, 'fullimage' => PT_ROOMS_IMAGES.$details[0]->thumbnail_image,'Images' => $images,'basicprice' => $basicprice,'salefrom' => $salefrom, 'saleto' => $saleto,'issale' => $issale,'saletype' => $saletype,'saleprice' => $saleprice,
            'Amenities' => $details['amenities'],'price' => $price,'currCode' => $curr->code,'currSymbol' => $curr->symbol,'Info' => $roomprice,'extraBeds' => $extrabeds,'extrabedCharges' => $bedcharges,
            'honeymoon' => $details[0]->honey_moon, 'room_title' => $details[0]->room_title, 'hfrom' => $details[0]->honey_moon_from, 'hto' => $details[0]->honey_moon_to);
           // }
        ...
        //make a result object of Rooms Array
         function getBookResultObject($hotelid,$roomid,$roomscount,$extrabeds = 0,$checkin = null,$checkout = null,$ishoney = null){
            ...
            else $totalSum = ($roomprice['totalPrice'] * $roomscount) + $extrabedcharges;
          ...
          $result["room"] = (object)array('id' => $roomid, 'title' => $this->roomtitle, 'desc' => $this->roomdesc,
            'maxAdults' => $details[0]->room_adults,'maxChild' => $details[0]->room_children,
            'maxQuantity' => $details[0]->room_quantity, 'thumbnail' => PT_ROOMS_THUMBS.$details[0]->thumbnail_image, 'Images' => $images,
            'Amenities' => $details['amenities'],'price' => $price,'currCode' => $curr->code,'currSymbol' => $curr->symbol,'perNight' => $perNight,'Info' => $roomprice,
            'stay' => $roomprice['stay'],'roomscount' => $roomscount,'extraBedCharges' => $bedcharges, 'extraBedsCount' => $extrabeds );
        //end room details for booking page


            $result["hotel"] = (object)array('id' => $this->hotelid, 'title' => $this->title, 'slug' => base_url().'hotels/'.$this->slug, 'bookingSlug' => $this->bookingSlug,'thumbnail' => $this->thumbnail,
            'stars' => pt_create_stars($this->stars),'starsCount' => $this->stars, 'location' => $this->location,'checkin' => $checkin, 'checkout' => $checkout,'mapAddress' => $this->mapAddress,
            'metadesc' => $this->metadesc,'keywords' => $this->keywords,'extras' => $extras,'taxAmount' => $taxAmount,'depositAmount' => $depositAmount,'serviceAmount' => $serviceAmount,
            'policy' => $this->policy,'extraChkUrl' => $extrasCheckUrl,'adults' => $this->adults,'children' => $this->children,'price_status' => $this->price_status,'hotel_policy' => $this->hotel_policy);
        ...
        //get updated values of booking data after extras and payment method updates
         function getUpdatedDataBookResultObject($hotelid,$roomid,$checkin,$checkout,$roomscount,$extras,$extrabeds = 0){
          ...
          $result = (object)array('grandTotal' => $price,'taxAmount' => $taxAmount,
            	...
            	'bookingType' => "hotels",'currCode' => $curr->code,
            	'currSymbol' => $curr->symbol,'subitem' => $subitem,'stay' => $roomprice['stay'],'extrasInfo' => $extratotal,'extraBedCharges' => $bedcharges);
		...
		public function suggestionResults($query){
        	...
        	foreach($locres as $l){
        		...
        		$locations[] = (object)array('id' => $l->id, 'name' => trim($locInfo->city),'module' => 'location');
        	}
        	...
        	foreach($res as $r){
        		...
        		$hotels[] =  (object)array('id' => $r->id, 'name' => trim($title), 'module' => 'hotel');
        	}
        ...
        foreach ($hotels as $hotel) {
            ...
            $locData->locations[] = (object) array('id' => $hotel->hotel_city, 'name' => $locationInfo->city, 'country' => $hotel->country, 'thumbnail_image' => $hotel->image, 'hotel_slug' => $hotel->hotel_slug, 'best' => $hotel->bestdesc);
            //unset($images);
            //$images = array();
        }
    ...
    // TripAdvisor Reviews Averages
		function tripAdvisorData($id, $tripInfo = null) {
			if(!empty($tripInfo)){
        		...
        		foreach($reviews as $rev){
        			...
        			$reviewsData[] = (object)array('id' => $rev->id,'review_comment' => $rev->text, 'review_name' => $rev->user->username,'review_overall' => $rev->rating,'maxRating' => 5,'reviewUrl' => $rev->url,'review_date' => strtotime($date[0]));
        		}

                $result = (object)array('totalReviews' => $info->num_reviews,'overall' => $info->rating,'imgUrl' => $info->rating_image_url,'reviews' => $reviewsData);
                 return $result;
		}
		...
		public function loadmoreHotels($locid) {
				...
				$this->db->where('id', $locid);
                                ...
                                $row = $query->row();
                                $result = (object)array('location'=> $row->location,'country'=> $row->country);
                                return $result;
		...
		public function hotelListings() {
            ...
            //array_push($images, $hotel->thumbnail_image);
            //$thumbnail_image = $images[array_rand($images)];
            $locData->locations[] = (object) array('hotel_id' => $hotel->hotel_id,'address' => $hotel->hotel_map_city, 'city' => $locationInfo->city, 'near' => $hotel->near, 'hotel_title' => $hotel->hotel_title);
            //unset($images);
            //$images = array();
        }
