...
class Uploads_model extends CI_Model {
 ...
 // upload profile picture
	...
	// upload location tour's picture

	function __locationtourimg()
	{
       ...
       $savethumb = PT_LOCATION_IMAGES_UPLOAD_THUMBS.$fig."_".$filename;
		$savefile = PT_LOCATION_IMAGES_UPLOAD.$fig."_".$filename;
	...
	/*end __location tour's img() */

	// upload location hotel's picture

	function __locationhotelimg()
	{

		$m = '';
		$fig = rand(1, 999999);
		$uploadedfile = $_FILES['hotelphoto']['tmp_name'];
		$filename = $_FILES['hotelphoto']['name'];
	...
	/*end __location hotel's img() */

	// Delete Location Hotel Images
	function delete_hotel_image($id) {
			$this->db->where('id', $id);
			$res = $this->db->get('pt_locations')->result();
			$img = $res[0]->hotelimage;
			if (!empty ($img)) {
					@ unlink(PT_LOCATION_IMAGES_UPLOAD . $img);
					@ unlink(PT_LOCATION_IMAGES_UPLOAD_THUMBS . $img);
			}
	}

	// Delete Location Tour Images
