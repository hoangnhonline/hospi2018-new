...
// Signup account
    function signup_account($type, $status) {
        ...
        $data = array('accounts_email' => $this->input->post('email'), 'accounts_password' => sha1($password), 'accounts_type' => $type, 'ai_title' => "", 'ai_first_name' => $firstname, 'ai_last_name' => $this->input->post('lastname'),'ai_address_1' => $address, 'ai_city' => $city, 'ai_country' => $this->input->post('country'), 'ai_mobile' => $this->input->post('phone'), 'accounts_created_at' => $now, 'accounts_updated_at' => $now,'accounts_verified' => $verified, 'accounts_status' => $status);
        $this->db->insert('pt_accounts', $data);
...
// My wishlist
    function my_wishlist($user) {
        ...
        $wishes = array();
        foreach ($res as $rs) {
            $lib = $rs->wish_module . "_lib";
            $this->load->library($rs->wish_module . "/" . $lib);
