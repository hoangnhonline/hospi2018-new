...
// Call the Model constructor
				...
				if ($mailsettings[0]->mail_default == "smtp") {
					...
					if($mailsettings[0]->mail_secure == "no"){
						...
						$config = Array('protocol' => 'smtp', 'charset' => 'utf-8',
							...
							'smtp_user' => $mailsettings[0]->mail_username, 'smtp_pass' => $mailsettings[0]->mail_password, 'mailtype' => 'html', 'charset' => 'utf-8', 'wordwrap' => TRUE,'smtp_auth' => TRUE);
		...
		function sendtestemail($toemail) {
                ...
                function senddatphongemail($email,$name,$phone,$msg) {
                                $this->email->set_newline("\r\n");
				$message = $this->mailHeader;
				$message .= "Nội dung:<br>".nl2br($msg)."<br><br>";
                                $message .= "Họ tên: ".$name."<br>";
                                $message .= "Email: ".$email."<br>";
                                $message .= "Số điện thoại: ".$phone."<br>";
				$message .= $this->mailFooter;
				$this->email->from($email);
				$this->email->to($this->sendfrom);
				$this->email->subject('Thông tin đặt phòng nhanh');
				$this->email->message($message);
				if (!$this->email->send()) {
						echo $this->email->print_debugger();
				}
                ...
                function sendlaygiaemail($email,$phone,$id,$duration) {
                                $this->load->library('hotels/hotels_lib');
                                $this->load->helper('text_helper');
                                $hoteld = $this->hotels_lib->hotel_details($id);
                                $roomsd = $this->hotels_lib->hotel_rooms($id);
                                //thumbnail
                                $star = "";
                                for($i=1;$i<=$hoteld->starsCount;$i++) {
                                    $star .= "★";
                                }
                                $gan = $hoteld->near;
                                if($hoteld->near=="") {
                                    $gan = "";
                                } else $gan = " - Gần ".$hoteld->near;
                                $desc = character_limiter($hoteld->desc,255);
                                $this->email->set_newline("\r\n");
				$message = $this->mailHeader;
                                $message .= "<table style='border-top:1px solid #660033;width:100%;padding-top: 20px;'><tbody><tr>";
                                $message .= "<td style='text-align:center;width: 25%;'><a style='text-decoration: none;' href='".base_url()."hotels/".$hoteld->slug."'><img alt='' src='".$hoteld->thumbnail."' width='150' /></a></td>";
                                $message .= "<td><strong><a style='text-decoration: none;' href='".base_url()."hotels/".$hoteld->slug."'>".$hoteld->title."</a></strong><br />".$hoteld->mapAddress."<br /><span style='color:#660033;'>".$star."</span> - ".$hoteld->location.$gan."<br>".$desc."<td></tr></tbody></table>";
                                if(!empty($hoteld->amenities)){ 
                                    $message .= "<table style='width:100%;padding: 20px;'><tbody><tr><td>";
                                            foreach($hoteld->amenities as $amt){ 
                                                if(!empty($amt->name)){ 
                                            
                                                    $message .= "<span style='border:1px solid #660033; color:#660033;border-radius:5px;padding:2px;margin-right:5px;'>".$amt->name."</span>"; 
                                ...
                                if (!empty($roomsd)) {
                                    $message .= "<table style='width:100%;padding: 20px;'><tbody>";
                                    $message .= "<tr><td style='border-top:1px solid #e2e2e2;width:60%;height:30px; '><b>Phòng</b></td><td colspan='2' style='height:30px; border-top:1px solid #e2e2e2;width:40%;'><b>Giá trung bình 1 đêm</b><br>".$duration."</td></tr>";
                                    foreach ($roomsd as $r) {
                                        $message .= "<tr><td style='border-top:1px solid #e2e2e2;height:30px; '>".$r->title."</td><td style='height:30px; border-top:1px solid #e2e2e2;'>".$r->price."</td><td style='height:30px; border-top:1px solid #e2e2e2;text-align:center;'><a style='border-radius: 5px;text-decoration: none;color:#ffffff;background-color:#660033;padding:5px 10px;' href='".base_url()."hotels/".$hoteld->slug."'>Đặt Phòng</a></td></tr>";
                                    }
                                    $message .= "</tbody></table>";
                                }
                                $message .= "<table style='width:100%;padding: 20px;'><tbody><tr><td style='border-top:1px solid #e2e2e2;padding-top: 20px;'>Thông tin của quý khách hàng:<br>Email: ".$email."<br>Điện thoại: ".$phone."</td></tr></tbody></table>";
                                
                                
				$message .= $this->mailFooter;
				$this->email->from($this->sendfrom);
				$this->email->to($email);
				$this->email->subject('Giá tốt tại '.$hoteld->title.' ('.$id.')');
				$this->email->message($message);
				if (!$this->email->send()) {
						echo $this->email->print_debugger();
				}
		...
		function sendEmail_customer($details, $sitetitle) {
			...
			$currencysign = $details->currSymbol;
				...
				$details = email_template_detail("bookingordercustomer");
                                ...
                                $dt = new DateTime();
                                $bookingtime = $dt->format('H:i')." phút ".$dt->format('d/m/Y');
                                 
                                 $total = number_format($totalamount);
				//$smsdetails = sms_template_detail("bookingordercustomer");
				$values = array($bookingtime,$paymentInfo,$paymentmethod,$thumbnail,$title,$address,$invoiceid, $refno, $deposit, $total, $sendto, $custid, $country, $phone, $currencycode, $currencysign, $invoicelink, $sitetitle, $duedate, $name);
				$message = $this->mailHeader;
...
// send New customer signup email to Admin
		...
		public function shortcode_variables($template) {
				...
				elseif ($template == "bookingordercustomer") {
                                                $result = array("{bookingtime}","{paymentInfo}","{paymentmethod}","{thumbnail}","{title}","{address}","{invoice_id}", "{invoice_code}", "{deposit_amount}", "{total_amount}", "{customer_email}", "{customer_id}", "{country}", "{phone}", "{currency_code}", "{currency_sign}", "{invoice_link}", "{site_title}", "{due_date}", "{fullname}");
				}
