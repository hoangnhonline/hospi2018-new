...
// Disable coupons
	public function add(){
		...
		if($_POST){
			if ($this->form_validation->run() == FALSE) {echo 123;echo  validation_errors() ;die;
				$this->data['response'] = array('status' => 'fail', 'msg' => '<div class="alert alert-danger">' . validation_errors() . '</div>');
			} else {
				...
				if( $couponid >0){
					redirect(base_url() . 'admin/coupons/index');
				}
			}
		}
		$this->data['main_content'] = 'modules/coupon/add';
		$this->load->view('template_new', $this->data);
	}
...
// generate coupon
	...
	public function edit(){
		$couponid = $this->input->get('id');
		$couponVo = $this->coupons_model->get_coupon_by_id($couponid);	
		$this->data['cop'] = $couponVo;
		$modules = array();
		$hotelsMod = pt_main_module_available('hotels');
		$toursMod = pt_main_module_available('tours');
		$carsMod = pt_main_module_available('cars');
		...
		$this->form_validation->set_rules('rate', 'Percentage', 'trim|required|is_numeric|greater_than[0]');
		$this->form_validation->set_rules('code', 'Coupon Code', 'trim|required|is_unique[pt_coupons.code]');*/
		$this->form_validation->set_rules('rate',  'trim|required|is_numeric|greater_than[0]');

		if($_POST){
			if ($this->form_validation->run() == FALSE) {				echo  validation_errors() ;die;
		...
		$this->data['main_content'] = 'modules/coupon/edit';
		$this->load->view('template_new', $this->data);
	}
}