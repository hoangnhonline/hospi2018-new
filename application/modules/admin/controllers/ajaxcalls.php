...
<div class='alert alert-danger'>" . trans("0313") . "</div>";
    ...
    function datphongnhanhEmail()
    {
        ...
        $this->load->model('admin/emails_model');
        echo $this->emails_model->senddatphongemail($email, $name, $phone, $msg);
    }

    function laygiaEmail()
    {
        $email = $this->input->post('email');
        $phone = $this->input->post('phone');
        $hotelid = $this->input->post('id');
        $title = $this->input->post('hotel');
        $this->load->model('admin/emails_model');
        echo $this->emails_model->sendlaygiaemail($email, $phone, $hotelid, $title);
    }
    ...
    {
        $accountlist = $this->input->post('items');
        foreach ($accountlist as $accid) {
            $this->accounts_model->delete_account($accid);
        }
        $this->session->set_flashdata('flashmsgs', "Deleted Successfully");
