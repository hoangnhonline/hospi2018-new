...
class Locations extends MX_Controller {
    ...
    $this->load->helper('xcrud');
		...
		$this->data['locations'] =  $this->locations_model->getLocationsByFilter();
		//$this->data['content'] = $xcrud->render();
		$this->data['page_title'] = 'Locations Settings';
		$this->data['main_content'] = 'location/index';
		...
		if(!empty($submit)){
			//Check whether user upload picture
           	if (isset ($_FILES['defaultphoto']) && !empty ($_FILES['defaultphoto']['name'])) {
					$tourimage = $this->uploads_model->__locationtourimg();
			}
			if (isset ($_FILES['hotelphoto']) && !empty ($_FILES['hotelphoto']['name'])) {
					$hotelimage = $this->uploads_model->__locationhotelimg();
			}
       		...
       		if($alreadyExists){
       			...
       			$this->locations_model->addLocation($tourimage,$hotelimage);
	...
	public function edit($id){
		...
		if(!empty($submit)){
			//Check whether user upload picture
            if (isset ($_FILES['defaultphoto']) && !empty ($_FILES['defaultphoto']['name'])) {
            	$this->uploads_model->delete_tour_image($id);
				$tourimage = $this->uploads_model->__locationtourimg();
			}
			if (isset ($_FILES['hotelphoto']) && !empty ($_FILES['hotelphoto']['name'])) {
				$this->uploads_model->delete_hotel_image($id);
				$hotelimage = $this->uploads_model->__locationhotelimg();
			}							
			$locid = $this->input->post('locationid');
			$this->locations_model->updateLocation($locid,$tourimage,$hotelimage);
			//redirect($this->segmentUrl.'/locations');
		...
		$this->data['countries'] = $this->countries_model->get_all_countries();	
       	$this->data['languages'] = pt_get_languages();
		$this->data['main_content'] = 'location/create';
		//$this->data['main_content'] = 'settings/locations';
		$this->data['page_title'] = $this->data['headingText'].' Location';
		$this->load->view('admin/template', $this->data);	
