...
class Honeymoon extends MX_Controller
{
    ...
    function index()
    {
        ...
        $this->data['info'] = array('base' => base_url() . 'admin/honeymoon', 'totalrows' => $total_records, 'perpage' => $limit);
        ...
        $this->data['page_title'] = 'Quản lý combo';
        $this->data['main_content'] = 'modules/honeymoon/index_new';
        $this->data['header_title'] = 'Quản lý combo';
    ...
    function add()
    {
        ...
        if (!empty($addoffer)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                redirect(base_url() . 'admin/honeymoon/index');
            }
        } else {
            $this->data['page_title'] = 'Thêm combo';
            //$this->data['main_content'] = 'modules/honeymoon/manage';
            $this->data['main_content'] = 'modules/honeymoon/add';
            $this->data['header_title'] = 'Quản lý combo';
    ...
    function manage($offerslug)
    {
        ...
        if (!empty($updateoffer)) {//var_dump($_POST["is_like"]);die;
            $this->form_validation->set_rules('offertitle', 'Offer Title', 'trim|required');
            $this->form_validation->set_rules('offerdesc', 'Offer Description', 'trim|required');
            if ($this->form_validation->run() == FALSE) {
                echo '<div class="alert alert-danger">' . validation_errors() . '</div><br />';
